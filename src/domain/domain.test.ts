...
describe('search', () => {
  const ranked = rankAll(RECIPES, { fridge: {}, prefs: { diet: 'Tout', allergies: [], cuisines: [], portions: 2 }, byId, today: '2026-09-23' })
  it('finds by recipe or ingredient name, without accents', () => {
    ...
    expect(searchRecipes(ranked, 'pois chiche', NO_FILTERS).map((e) => e.recipe.id)).toEqual(['r8'])
  })
  it('applies the filter pills', () => {
    expect(searchRecipes(ranked, '', { ...NO_FILTERS, quick: true }).every((e) => e.recipe.minutes <= 20)).toBe(true)
    expect(searchRecipes(ranked, '', { ...NO_FILTERS, cuisine: 'Mexicaine' }).map((e) => e.recipe.id).sort()).toEqual(['r16', 'r22'])
